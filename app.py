...
@app.route("/SenCla", methods=["POST", "GET"])
def predict_sentiment():
    text = request.json["text"]
    print(text)
    res_val = cat(text)
    key = [key for (key, value) in pl.vocabulary.items() if value == res_val]
    res_wod = key[0]
    return jsonify({"category_ind": res_val, "category_tex": res_wod})
